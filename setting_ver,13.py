    ...
    def draw_board(self):
        ...
        if self.settings.get("flip_limit_event"):
            for pi in range(len(self.players)):
                ...
                if self.flip_mission_counters[pi] == 0:
                    text.append(f"{p_name}の反転ﾐｯｼｮﾝ(今!): {self.flip_mission_targets[pi]}個")
                else:
                    text.append(f"{p_name}の反転ﾐｯｼｮﾝ(あと{self.flip_mission_counters[pi]}回): {self.flip_mission_targets[pi]}個")
