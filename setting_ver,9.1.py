...
class OthelloApp:
    ...
    def add_event_section(self):
        ...
        # スクロール機能のコアとなるCanvasとScrollbarを作成
        canvas = tk.Canvas(frame, highlightthickness=0)
        scrollbar = tk.Scrollbar(frame, orient=tk.VERTICAL, command=canvas.yview)
        
        # 実際に各イベント行を配置する内側のFrame
        inner_frame = tk.Frame(canvas)
        
        # 内側フレームのサイズが変わった際に、Canvasのスクロール可能領域(scrollregion)を更新する
        inner_frame.bind(
            "<Configure>",
        ...
        # Scrollbarを右端に、Canvasを残り全体に配置
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        # 【重要】各イベントの配置先(親ウィジェット)を frame ではなく inner_frame に変更しています
        ...
        self.event_row(inner_frame, "盤面拡大", "expand", [("回数", "expand_count", 1, 10)])
        ...
        self.event_row(inner_frame, "反転個数指定", "flip_limit_event", [("間隔", "flip_limit_interval", 0, 99)])
        self.event_row(inner_frame, "反転力低下", "flip_limit", [("上限", "flip_limit_count", 1, 99)])
        self.event_row(inner_frame, "判定逆転", "reverse_judgment", [])
        self.event_row(inner_frame, "総得点オセロ", "total_score", [("上限得点", "total_score_count", 1, 99)], "total_score_show", "表示")
    ...
    def draw_grid(self, board, title=None):
        ...
        for r in range(rows):
            for c in range(cols):
                ...
                if self.settings.get("total_score_show") and value != HOLE and hasattr(self, "cell_scores"):
                    if size >= 20: # マスがある程度大きいときだけ表示
                        self.canvas.create_text(
                            x1 + 4, y1 + 3,         # 左上の端っこ
